...
function generate_pooling_problem(num_sources::Int, num_pools::Int, num_terminals::Int, num_k::Int, 
    ...
    """
    Generates a GAMS file for a randomized pooling problem instance matching the described generation process.
    ...
    - Arc costs: c_ij = d_i - d_j if arc exists (0 otherwise).
      ...
      - d_i ~ Uniform{Integer} {10,...,30} for i ∈ T.
    ...
    # Generate capacities
    bl = zeros(Int, total_i)  # All 0
    bu = [rand(20:80) for _ in 1:total_i]
    
    # Generate adjacency a(i,j) and costs c(i,j)
    a = zeros(Int, total_i, total_i)
    c = zeros(Float64, total_i, total_i)  # Use Float64 for formatting
    for i in from_nodes
        # Determine possible j based on no_direct_st
        if i in sources
            possible_j = no_direct_st ? pools : to_nodes  # NEW: Restrict S to I only if true
        else  # i in pools
...
for i in 1:5
    generate_pooling_problem(30, 40, 30, 4, 0.4,"K$i.gms", acyclic=true, seed=i*100)
